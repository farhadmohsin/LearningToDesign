...
for t in range(profiles):
    ...
    # df_uf = df_uf.append(pd.Series(np.concatenate((uf,[w, wfair, gen, exist]))), ignore_index=True)
    
    # toc = time()
    # print(f"Time to generate*24: {toc-tic} s")
    # tic = time()
    # for pi in perms[1:]:
...
print(cnt_special, uf_special, uf_special/cnt_special)
print(cnt_special2, uf_special2, uf_special2/cnt_special2)
print(cnt_special3, uf_special3, uf_special3/cnt_special3)
...
# df = pd.read_csv('xgboostFairInput1591244578.csv')
...
for beta in np.arange(0.1,0.8,0.1):
    ...
    for i,ix in enumerate(test_index):
        ...
        # 74 = m*2 + m*2 + m*2 + m*2 + 2 + 4 + 4
        # x(64), n1, n2, [w,wfair,gen,exist],uf
        if(val[69] and val[68]):
            if(predictions3[i] == pi[int(val[66])]):
                acc_Cond += 1
            tot_Cond += 1
        if((val[68])):
            uf_og = val[70:74]
