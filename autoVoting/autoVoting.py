...
def monotonic_sample_gen(pwp):
    ...
    for i,v in enumerate(ballots):
        ...
        if(w_idx > 0):
            ...
            #   put winner at top, bring everyone else down
            temp_ballots = np.zeros(ballots[i].shape)
            temp_ballots[0] = w
            for k in range(1,w_idx+1):
                temp_ballots[k] = ballots[i][k-1]
            for k in range(w_idx+1,len(ballots[i])):
                temp_ballots[k] = ballots[i][k]
            
            ballots[i] = temp_ballots
